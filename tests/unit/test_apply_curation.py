...
def _write_curation_csv(
    ...
    *,
    ...
    cols = (
        "dagger_code", "dagger_label", "asterisk_code", "asterisk_label",
        "combination_labels", "levels_present",
        "redundancy_level", "rationale", "curated_by", "curated_date",
        "_orphan",
    )
    defaults: dict[str, object] = {
        "dagger_code": "",
...
def test_apply_curation_autodetects_semicolon_separator(tmp_path: Path) -> None:
    ...
    curated, report = dagger_asterisk.apply_curation(table, csv)
    assert curated.filter(pl.col("dagger_code") == "A17.8").row(0, named=True)[
        "redundancy_level"
    ] == "subordinate"
...
def test_apply_curation_orphan_flagged_pair_is_ignored(tmp_path: Path) -> None:
    ...
    _write_curation_csv(csv, [
        {
            "dagger_code": "A17.8", "asterisk_code": "G05.0",
            "redundancy_level": "subordinate", "_orphan": "true",
        },
    ])
    curated, report = dagger_asterisk.apply_curation(table, csv)
