...
def test_synonymes_ofs_priority_dedup() -> None:
    ...
    out = _df(
        flat_csv.build(
            merged, _make_propagated([]), _make_siblings([]), owl, ofs, _make_dagger_asterisk([])
        )
    )
...
def test_synonymes_ofs_parens_normalized() -> None:
    merged = _make_merged(
        [{"code": "A00-A09", "label": "Bloc", "type": "block", "left": 1, "right": 2}]
    )  # techniquement non-leaf, mais on teste juste la normalisation
    # → on contourne en utilisant un leaf
    merged = _make_merged(
        [{"code": "A00-A09", "label": "Bloc", "type": "category", "left": 1, "right": 2}]
    )
        ...
        [
            {
                "code": "A00.0",
                "note_type": "exclusion",
                "texte": "héritée du bloc",
                "source": "OFS",
                "inherited_from": "A00-A09",
                "inherited_from_type": "block",
            },
        ]
    )
    out = _df(
...
def test_libelle_attached_correctly() -> None:
    merged = _make_merged(
        [{"code": "A00.0", "label": "Choléra à V. cholerae", "left": 1, "right": 2}]
    )
    propagated = _make_propagated(
        [
...
def test_sort_order_type_inclusion_first() -> None:
    ...
    out = _df(
        flat_csv.build(
            merged, propagated, _make_siblings([]), owl, _make_ofs([]), _make_dagger_asterisk([])
        )
    )
...
def test_flat_csv_is_dagger_in_pair_true_for_dagger_code() -> None:
    ...
    out = _df(
        flat_csv.build(merged, propagated, _make_siblings([]), _make_owl([]), _make_ofs([]), dag)
    )
...
def test_filter_redundant_dagger_synonym_dropped() -> None:
    ...
    out = _df(
        flat_csv.build(merged, _make_propagated([]), _make_siblings([]), _make_owl([]), ofs, dag)
    )
    # Le synonyme survit uniquement côté astérisque (N33.0).
    syn_dagger = out.filter((pl.col("code") == "A18.1") & (pl.col("type") == "synonyme"))
    syn_aster = out.filter((pl.col("code") == "N33.0") & (pl.col("type") == "synonyme"))
