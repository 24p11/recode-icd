...
_KIT = (
    ...
    "W0004 |2|NNO|1|CHUTE GLACE RUE|Chute de plain-pied due \xe0 la glace et la neige, rue ou route\r\n"
...
def test_load_atih_kit_lit_le_format(kit_path: Path) -> None:
    ...
    assert df["type_mco"].to_list() == [3, 0, 0, 1, 0, 0, 0, 2, 3, 4]
...
def test_to_parquet_ecrit_metadonnees_et_rapport(kit_path: Path, tmp_path: Path) -> None:
    ...
    assert rapport.filter(pl.col("valeur") == "codes")["count"][0] == 10
    assert rapport.filter(pl.col("valeur") == "codables_mco")["count"][0] == 8
